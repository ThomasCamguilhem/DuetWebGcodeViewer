...
window.onload = function() {
	...
	$("#read").click(function () {
		...
		*/
		...
		if (PREVIEW)
		{
			preview = new THREE.Geometry();
			var previewMaterial = new THREE.LineBasicMaterial({color: 0x000000});
			var previewLines = new THREE.LineSegments( preview, previewMaterial );
			previewLines.name = "preview";
...
{	
	...
	if (lay != pointCloud[keys[i]].length )
	{
		while(pointCloud[keys[i]][lay] == undefined)
			lay++;
		if (pointCloud[keys[i]][lay] != undefined)
			renderGeo(keys[i], lay)
		lay ++;
