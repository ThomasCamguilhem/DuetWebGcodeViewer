...
var saveFile = function (strData, filename) {
	var link = document.createElement('img')//('a');
	//if (typeof link.download === 'string') {
		document.body.appendChild(link); //Firefox requires the link to be in the body
		link.alt/*.download*/ = filename;
		link.src/*href*/ = strData;
		link.click();
			...
			for (var layer in pointCloud[key])
			{
				switch (slicer)
				{
					...
					case Slicer.SIMP:
						...
						break;						
					case Slicer.SLIC:
						scene.getObjectByName(key+"_"+layer).visible = (($("#"+key.toLowerCase()+"_slic")[0].checked) && (thisLay >= layStart && thisLay <= layEnd));
						break;
				}
				thisLay++;
			}
