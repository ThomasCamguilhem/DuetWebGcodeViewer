		...
		case "G1":
			...
			if (zlayer != lastPos.z && extruding)
			{
				...
				if (layers[curLay-1])
				{
					...
					geo.vertices.push(new THREE.Vector3(-layers[curLay-1].layer.lBBox.min.x, lastPos.z, layers[curLay-1].layer.lBBox.min.y))
					geo.vertices.push(new THREE.Vector3(-layers[curLay-1].layer.lBBox.min.x, lastPos.z, layers[curLay-1].layer.lBBox.max.y))
					geo.vertices.push(new THREE.Vector3(-layers[curLay-1].layer.lBBox.max.x, lastPos.z, layers[curLay-1].layer.lBBox.max.y))
					geo.vertices.push(new THREE.Vector3(-layers[curLay-1].layer.lBBox.max.x, lastPos.z, layers[curLay-1].layer.lBBox.min.y))
				...
				if (SHOW_ALL)
				{
					...
					if(PREVIEW) {
						...
						{
							var color = new THREE.Color().setHSL(((i-1)/nbKey), 0.75, 0.5);
							//console.log(key +" ("+color.r+","+color.g+","+color.b+")")
							pointMaterial = new THREE.LineBasicMaterial({color: color, weight:2});
							meshMaterial = new THREE.MeshPhongMaterial({color: color});
							if (pointCloud[key][curLay-1] != undefined)
							{
								pointCloud[key][curLay-1].computeVertexNormals();
								if (fileSize < 10*1024*1024)
								{
									threeDee = (key != "MOVE"?new THREE.Mesh(
										new THREE.BufferGeometry().fromGeometry(pointCloud[key][curLay-1])
										, meshMaterial )
										: new THREE.LineSegments(pointCloud[key][curLay-1]
										, pointMaterial ))
								} else {
									threeDee =  new THREE.LineSegments(pointCloud[key][curLay-1]
									, pointMaterial )
								}
								threeDee.castShadow = true;
								threeDee.receiveShadow = true;
								threeDee.name = key+"_"+(curLay-1);
								scene.add( threeDee );
							}
							i++;
						}
						...
						try {
							...
							//saveFile(imgData.replace(strMime, strDownloadMime), fileInput.name.substring(0,fileInput.name.lastIndexOf("."))+"_"+(curLay-1)+".jpg");
