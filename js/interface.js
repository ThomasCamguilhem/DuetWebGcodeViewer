		...
		toolMapping.forEach(function(tool) {
			...
			if (tool.heaters.length == 0) {
				...
				tool.heaters.forEach(function(heater) {
					...
					row +=		'<td class="input-td visible-sm visible-xs"><div class="input-group" style="display: inline-flex;">';
					row +=			'<button class="btn btn-default decrement-button" onclick="decrement(this)">−</button>'
					row +=			'<input type="number" class="form-control" value="' + activeTemp + '" data-type="active" ondblclick="showTemp(this, \''+toolName+'\',\'active\')" disabled>';
					row +=			'<button class="btn btn-default increment-button" onclick="increment(this)">+</button>';
					row +=		'</div></td>';
					row +=		'<td class="input-td hidden-sm hidden-xs"><div class="input-group">';
					row +=			'<input type="number" class="form-control" value="' + activeTemp + '" data-type="active" disabled>';
					row +=			'<div class="input-group-btn div-head-temp">';
					...
					row +=		'<td class="input-td visible-sm visible-xs"><div class="input-group" style="display: inline-flex;">';
					row +=			'<button class="btn btn-default decrement-button" onclick="decrement(this)">−</button>'
					row +=			'<input type="number" class="form-control standby-input" value="' + standbyTemp + '" data-type="standby" ondblclick="showTemp(this, \''+toolName+'\',\'standby\')" disabled>';
					row +=			'<button class="btn btn-default increment-button" onclick="increment(this)">+</button>'
					row +=		'</div></td>';
					row +=		'<td class="input-td hidden-sm hidden-xs"><div class="input-group">';
...
$("#table_tools").on("keydown", "tr > td > div > input", function(e) {
	...
	if ((isConnected && enterKeyPressed) || e.which === undefined) {
		var tool = $(this).closest("tr").data("tool");
...
function btnMoveMouseDownEvent(target){
	...
	if (!target.includes("crawl"))
	{
		...
		setTimeout( function() {
			btnMoveMouseUpEvent(target.replace("crawl", "fast"), true);
		}, 50);
	}
}
...
$("#div_content").resize(function() {
	...
	if($("#div_static_sidebar").first().length)
