		...
		success: function(response) {
			if (isConnected) {
				if (response.hasOwnProperty("err")) {
					...
					if (response.next != 0 && !compatibilityMode && response.next != undefined) {
...
function addGCodeFileMiniature(filename) {
	...
	//row +=		'<td><input type="checkbox"></td>';
	row.innerHTML +=	'<td draggable="true" class="name" style="width: 175px;" data-file="' + filename + '"><span class="glyphicon glyphicon-asterisk"></span> ' + filename.replace(/_/g,' ') + '</td>';
	return $(row).appendTo("#table_gcode_files > tbody");
}
...
function addGCodeDirectoryMiniature(name) {
	...
	row.innerHTML += '<td draggable="true" data-directory="' + name + '"><div style="width: 155px; margin: 0 14%;"><a href="#" class="a-gcode-directory"><img src="img/folder.svg" width="150" ><br/> ' + name + '</a></div></td>';
			...
			if (isConnected) {
				if (response.err != 0) {
					...
					if (response.files.length == 0) {
						...
						if (response.next != 0 && response.next != undefined) {
...
$("body").on("click", ".a-gcode-file", function(e) {
	...
	console.log( ajaxPrefix + "img/GCodePreview/" + dirName + "/" + fileName)
	$("#modal_confirmation_img")[0].alt = dirName + "/" + fileName;
...
function addMacroFile(filename, size, lastModified) {
	...
	if (currentMacroDirectory == "0:/macros") {
		...
		macroButton +=		'<button class="btn btn-default btn-macro ' +(window.innerWidth < 768?'btn-xl':(window.innerWidth < 991?'btn-lg':''))+ '" data-macro="0:/macros/' + filename + '">' +  stripMacroFilename(filename) + '</button>';
		macroButton +=		'</div>';
...
function addMacroDirectory(name) {
	...
	if (currentMacroDirectory == "0:/macros") {
		...
		button +=		'<button class="btn btn-info btn-macro ' +(window.innerWidth < 768?'btn-xl':(window.innerWidth < 991?'btn-lg':''))+ '" data-directory="0:/macros/' + name + '" data-toggle="dropdown">' + name + ' <span class="caret"></span></button>';
...
function sortTableTd(table) {
	...
	var body = table.children("tbody");
	var colNum = Math.ceil(window.innerWidth/200);
	var rows = Math.ceil(cols.length/colNum);
