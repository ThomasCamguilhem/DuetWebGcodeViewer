...
var ajaxPrefix = "http://192.168.1.40/";					// Host to use for AJAX requests + "?" char or empty if running on a webserver
...
function connect(password, regularConnect) {
	if (regularConnect) {
		...
		if (location.host == "" || location.host == "localhost" || location.host == "127.0.0.1") {
			showHostPrompt();
			return;
		}
	}
...
function updateStatus() {
	...
	$.ajax(ajaxPrefix + ajaxRequest, {
		...
		success: function(status) {
			...
			if ($("td[data-axis='" + 0 + "']").html() != "n/a" && $("td[data-axis='" + 1 + "']").html() != "n/a" && false)
			{
...
function getConfigResponse(sync) {
		...
		success: function(response) {
			if (response != "") {
				...
				if (response.hasOwnProperty("vendor")) {
					if (response.vendor == "diabase") {
						setOem(response.vendor);
					} else if (response.vendor == "lynxter") {
						setLynxter(response)
					}
				}
			}
...
function setLynxter (config) {
	...
	$(".navbar-brand > img").removeClass("hidden");
		...
		$(".tab_material").removeClass("hidden");
		$("#page_material").removeClass("hidden");
