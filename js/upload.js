...
var parseType, parseFiles, parseRows, parsedFileCount;
...
function startUpload(type, files, fromCallback) {
	...
	parseTotalBytes = parsedTotalBytes = parsedFileCount = 0;
	uploadFiles = files;
	parseFiles = [];
	$.each(files, function() {
	...
	$("#table_parse_files > tbody").children().remove();
	$.each(files, function() {
		if (type == "generic") {
		...
		if (type == "gcode" || type == "start"  || type == "preview")
		{
			...
			rowElem = $(row);
			rowElem.appendTo("#table_parse_files > tbody");		
			parseRows.push(rowElem);
			parseFiles.push(this);
		}
	});
